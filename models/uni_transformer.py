...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_type_feat, edge_index, mask_ligand, e_w=None, fix_x=False):
        src, dst = edge_index
        ...
        for i in range(self.num_x2h):
            h_out = self.x2h_layers[i](h_in, base_dist_feat, edge_feat, edge_index, e_w=e_w)
            h_in = h_out
        x2h_out = h_in
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def forward(self, h, x, mask_ligand, batch,
                bond_edge_index=None,   # ✅ 新增：bond 图
                bond_edge_attr=None,       # [E_b, bond_dim]
        ...
        for _ in range(self.num_blocks):
            ...
            if bond_edge_index is not None:
                ...
                edge_type_idx = torch.ones(E_r, device=x.device, dtype=torch.long)  # all radius

            src, dst = edge_index

            # 3) edge_attr used by outer_product(edge_attr, dist_feat) => expects dim=2
            edge_type_feat = F.one_hot(edge_type_idx, num_classes=2).float() # [E_b+E_r, 2]

            # 4) merge with bond edge_attr (pad zeros for radius edges)
            if bond_edge_attr is not None:
                bond_edge_attr = bond_edge_attr.to(x.device).float()
                bond_dim = bond_edge_attr.size(-1)

                # bond edges have true attrs, radius edges padded with zeros
                bond_pad = torch.zeros((E_r, bond_dim), device=x.device, dtype=bond_edge_attr.dtype)
                bond_feat_union = torch.cat([bond_edge_attr, bond_pad], dim=0)  # [E_b+E_r, bond_dim]

                edge_attr = torch.cat([edge_type_feat, bond_feat_union], dim=-1)  # [E, 2+bond_dim]
                assert edge_attr.shape[-1] == self.edge_feat_dim
            else:
                # fallback: only edge type feature
                edge_attr = edge_type_feat  # [E,2]

            # 5) optional global edge weight
            if self.ew_net_type == 'global':
                dist = torch.norm(x[dst] - x[src], p=2, dim=-1, keepdim=True)  # computed on noisy x
                dist_feat = self.distance_expansion(dist) # [E, num_r_guassian]
                logits = self.edge_pred_layer(dist_feat) # [E, hidden_dim]
                e_w = torch.sigmoid(logits)  # [E,hidden_dim]
            else:
                e_w = None

            # 6) stacked equivariant layers
            for layer in self.base_block:
                h, x = layer(h, x, edge_attr, edge_type_feat, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x)
