...
class NodeBlock(Module):
    ...
    def __init__(self, node_dim, edge_dim, hidden_dim, use_gate, graph_cond_dim=0):
        super().__init__()
        ...
        if self.graph_cond_dim > 0:
            self.adaLN = AdaLN(hidden_dim, cond_dim=graph_cond_dim+1)
        else:
    ...
    def forward(self, x, edge_index, edge_attr, node_time, graph_cond=None):
        """
        Args:
            x:  Node features, (N, H).
            edge_index: (2, E).
        ...
        if self.graph_cond_dim > 0:
            cond = torch.cat([graph_cond, node_time], dim=-1)
            out = self.adaLN(out, cond)
        else:
...
class EdgeBlock(Module):
    def __init__(self, edge_dim, node_dim, hidden_dim=None, use_gate=True, graph_cond_dim=0):
        super().__init__()
        self.use_gate = use_gate
        ...
        self.graph_cond_dim = graph_cond_dim
        if self.graph_cond_dim > 0:
            self.adaLN = AdaLN(edge_dim, cond_dim=graph_cond_dim+1)
        else:
            self.layer_norm = nn.LayerNorm(edge_dim)
        self.out_transform = Linear(edge_dim, edge_dim)
        self.act = nn.ReLU()

    def forward(self, h_bond, bond_index, h_node, bond_time, graph_cond=None):
        """
        h_bond: (b, bond_dim)
        bond_index: (2, b)
        h_node: (n, node_dim)
        ...
        if self.graph_cond_dim > 0 and graph_cond is not None:
            cond = torch.cat([graph_cond, bond_time], dim=-1)
            h_bond = self.adaLN(h_bond, cond)
        else:
            h_bond = self.layer_norm(h_bond)
...
class NodeEdgeNet(Module):
    def __init__(self, node_dim, edge_dim, num_blocks, cutoff, use_gate, graph_cond_dim=0, **kwargs):
        super().__init__()
        self.node_dim = node_dim
        self.edge_dim = edge_dim
        self.num_blocks = num_blocks
        ...
        for _ in range(num_blocks):
            self.node_blocks_with_edge.append(NodeBlock(
                node_dim=node_dim, edge_dim=edge_dim, hidden_dim=node_dim, use_gate=use_gate, graph_cond_dim=graph_cond_dim,
            ))
            self.edge_embs.append(Linear(input_edge_dim, edge_dim))
            if self.update_edge:
                self.edge_blocks.append(EdgeBlock(
                    edge_dim=edge_dim, node_dim=node_dim, use_gate=use_gate, graph_cond_dim=graph_cond_dim,
                ))
            if self.update_pos:
    ...
    def forward(self, 
                h_node, pos_node, h_edge, edge_index, node_time, edge_time,
                node_cond=None, edge_cond=None, graph_cond_node=None, graph_cond_edge=None):
        ...
        for i in range(self.num_blocks):
            ...
            if node_cond is not None:
                h_node_eff = h_node + node_cond
            if edge_cond is not None:
                h_edge_eff = h_edge + edge_cond

            h_node_with_edge = self.node_blocks_with_edge[i](h_node_eff, edge_index, h_edge, node_time, graph_cond=graph_cond_node)
            if self.update_edge:
                h_edge = h_edge + self.edge_blocks[i](h_edge_eff, edge_index, h_node, edge_time, graph_cond=graph_cond_edge)
