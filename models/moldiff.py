...
from models.denoiser import NodeEdgeNet
...
class MolDiff(Module):
    def __init__(self,
        ...
        **kwargs
        ...
        super().__init__()
        self.config = config
        self.num_node_types = getattr(config.data, 'num_node_types', 35)
        self.num_edge_types = getattr(config.data, 'num_edge_types', 4) + 1
        self.bond_len_loss = getattr(config, 'bond_len_loss', False)

        # # define beta and alpha
        self.define_betas_alphas(config.model.diff)

        # # embedding
        node_dim = config.model.node_dim
        edge_dim = config.model.edge_dim
        time_dim = config.model.diff.time_dim
        self.node_embedder = nn.Linear(self.num_node_types, node_dim-time_dim, bias=False)  # element type
        self.edge_embedder = nn.Linear(self.num_edge_types, edge_dim-time_dim, bias=False) # bond type
        self.time_emb = nn.Sequential(
            GaussianSmearing(stop=self.num_timesteps, num_gaussians=time_dim),
        )

        # condition 
        self.cond_drop = getattr(config.train, 'cond_dropout', None)
        self.cond_node_dim = config.encoder.hidden_dim
        self.cond_graph_dim = config.encoder.hidden_dim
        self.cond_edge_dim = config.encoder.hidden_dim

        self.node_cond_proj = nn.Linear(self.cond_node_dim, node_dim, bias=False)
        self.graph_cond_proj = nn.Linear(self.cond_node_dim, self.cond_graph_dim, bias=False)
        edge_in = 3 * self.cond_node_dim  # (ci+cj, ci*cj, |ci-cj|)
        self.edge_cond_proj = MLP(edge_in, edge_dim, hidden_dim=config.model.edge_dim)

        self.node_null = nn.Parameter(torch.zeros(self.cond_node_dim))
        self.graph_null = nn.Parameter(torch.zeros(self.cond_graph_dim))
                
        # # denoiser
        if config.model.denoiser.backbone == 'NodeEdgeNet':
            self.denoiser = NodeEdgeNet(node_dim, edge_dim, **config.model.denoiser)
        else:
            raise NotImplementedError(config.model.denoiser.backbone)
    ...
    def apply_dropout(self, node_cond, graph_cond, batch_node, num_graphs):
        if self.cond_drop is None:
            return node_cond, graph_cond
        cfg_drop = self.cond_drop
        device = batch_node.device
        B = num_graphs

        if node_cond is None:
            node_cond = self.node_null[None, :].expand(batch_node.size(0), -1)
        if graph_cond is None:
            graph_cond = self.graph_null[None, :].expand(B, -1)
        
        r = torch.rand(B, device=device)

        uncond = r < cfg_drop["p_uncond"]
        drop_node = (~uncond) & (r < cfg_drop["p_uncond"] + cfg_drop["p_drop_node"])
        drop_graph = (~uncond) & (r < cfg_drop["p_uncond"] + cfg_drop["p_drop_node"] + cfg_drop["p_drop_graph"])

        # drop graph (per-graph)
        graph_cond = graph_cond.clone()
        graph_cond[uncond | drop_graph] = self.graph_null

        # drop node (broadcast by batch_node)
        node_cond = node_cond.clone()
        node_mask = (uncond | drop_node)[batch_node]   # [N]
        node_cond[node_mask] = self.node_null

        return node_cond, graph_cond
    
    def get_loss(self, batch, node_cond=None, graph_cond=None,
    ):
        # node_type, node_pos, batch_node,
        #         halfedge_type, halfedge_index, batch_halfedge,
        #         num_mol,
        node_type = batch.x[:,0].long() # [N]
        node_pos = batch.pos # [N, 3]
        batch_node = batch.batch

        # complete graph halfedges
        halfedge_index, halfedge_type, batch_halfedge, num_graphs = self.build_complete_halfedges(batch)
        device = node_pos.device
        ...
        node_cond, graph_cond = self.apply_dropout(node_cond, graph_cond, batch_node, num_graphs)
        preds = self(
            h_node_pert, pos_pert, batch_node,
            h_edge_pert, edge_index, batch_edge, 
            t=time_step, 
            node_cond=node_cond, graph_cond=graph_cond,
        )
        ...
        if self.categorical_space == 'discrete':
            ...
            log_edge_post_pred = self.edge_transition.q_v_posterior(log_edge_recon, log_halfedge_t, time_step, batch_halfedge, v0_prob=True)
            kl_edge = self.edge_transition.compute_v_Lt(log_edge_post_true, log_edge_post_pred, log_halfedge_0,  t=time_step, batch=batch_halfedge)
    ...
    def forward(self, h_node_pert, pos_pert, batch_node,
                ...
                node_cond=None, graph_cond=None):
        """
        ...
        # 1.5 condition embedding
        node_cond_emb = self.node_cond_proj(node_cond)
        graph_cond_emb = self.graph_cond_proj(graph_cond)
        node_cond_left, node_cond_right = node_cond[edge_index[0]], node_cond[edge_index[1]]
        edge_cond = torch.cat([node_cond_left + node_cond_right, node_cond_left * node_cond_right, torch.abs(node_cond_left - node_cond_right)], dim=-1)
        edge_cond_emb = self.edge_cond_proj(edge_cond)

        # 2 diffuse to get the updated node embedding and bond embedding
        graph_cond_node = graph_cond_emb.index_select(0, batch_node)
        graph_cond_edge = graph_cond_emb.index_select(0, batch_edge)
        h_node, pos_node, h_edge = self.denoiser(
            ...
            node_cond=node_cond_emb, edge_cond=edge_cond_emb,graph_cond_node=graph_cond_node, graph_cond_edge=graph_cond_edge,
        )
    ...
    @torch.no_grad()
    def sample(self, n_graphs, batch_node, halfedge_index, batch_halfedge, bond_predictor=None, guidance=None, node_cond=None, graph_cond=None,cfg_drop=None,):
        device = batch_node.device
    ...
    def build_complete_halfedges(self, batch):
        ptr = batch.ptr
        batch_node = batch.batch
        ...
        bond_type = batch.edge_attr[:, 0].long() + 1  # 1..4，0 留给 no-bond
        g_edge = batch_node[src]                      # 每条原始边属于哪个图
        ...
        for gi in range(num_graphs):
            start = int(ptr[gi].item())
            end = int(ptr[gi + 1].item())
            n = end - start
            if n < 2:
                continue

            # 完全图无向 pair（local）
            pairs = torch.combinations(torch.arange(n, device=device), r=2)  # [nC2,2]

            # 用 n×n 表填真实 bond_type（默认 0 = no-bond）
            mat = torch.zeros((n, n), dtype=torch.long, device=device)
            ...
            if m.any():
                ...
                b = torch.maximum(u, v)
                keep = (a != b)
                mat[a[keep], b[keep]] = bond_type[m][keep]  # 1..4

            types = mat[pairs[:, 0], pairs[:, 1]]                 # [nC2]
            idx_global = (pairs + start).t().contiguous()         # [2,nC2]

            halfedge_index_list.append(idx_global)
            halfedge_type_list.append(types)
            batch_halfedge_list.append(torch.full((pairs.size(0),), gi, dtype=torch.long, device=device))

        halfedge_index = torch.cat(halfedge_index_list, dim=1)
        halfedge_type = torch.cat(halfedge_type_list, dim=0)
        batch_halfedge = torch.cat(batch_halfedge_list, dim=0)
        return halfedge_index, halfedge_type, batch_halfedge, num_graphs
